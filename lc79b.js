...
const normalize = item => ({
    ket_qua: (item.resultTruyenThong || '').toLowerCase(),
    tong: item.point || 0,
...
class TheGodPredictor {
    ...
    streak_basic() { if (this.kqSeq.length < 3) return null; const last = this.kqSeq[this.kqSeq.length - 1]; let s = 1; for (let i = this.kqSeq.length - 2; i >= 0 && this.kqSeq[i] === last; i--) s++; if (s >= 2) return { pred: last === 1 ? "Tài" : "Xỉu", conf: Math.min(85, 55 + s * 4) }; return null; }
    streak_break() { if (this.kqSeq.length < 5) return null; const last = this.kqSeq[this.kqSeq.length - 1]; let s = 1; for (let i = this.kqSeq.length - 2; i >= 0 && this.kqSeq[i] === last; i--) s++; if (s >= 4) return { pred: last === 1 ? "Xỉu" : "Tài", conf: Math.min(75, 65 + (s - 4) * 2) }; return null; }
    ...
    pattern_1_2_3() { if (this.kqSeq.length < 6) return null; const l = this.kqSeq.slice(-6); if (l[0] === 1 && l[1] === 0 && l[2] === 0 && l[3] === 1 && l[4] === 1 && l[5] === 1) return { pred: "Xỉu", conf: 65 }; if (l[0] === 0 && l[1] === 1 && l[2] === 1 && l[3] === 0 && l[4] === 0 && l[5] === 0) return { pred: "Tài", conf: 65 }; return null; }
    pattern_3_nhip() { if (this.kqSeq.length < 6) return null; const l = this.kqSeq.slice(-6); if (l[0] === 1 && l[1] === 0 && l[2] === 1 && l[3] === 0 && l[4] === 1 && l[5] === 0) return { pred: "Xỉu", conf: 68 }; if (l[0] === 0 && l[1] === 1 && l[2] === 0 && l[3] === 1 && l[4] === 0 && l[5] === 1) return { pred: "Tài", conf: 68 }; return null; }
    ...
    fibonacci_retracement() { if (this.kqSeq.length < 20) return null; const r = this.kqSeq.slice(-20); const peak = Math.max(...r); const trough = Math.min(...r); if (peak - trough > 0) { const fib382 = trough + 0.382 * (peak - trough); const fib618 = trough + 0.618 * (peak - trough); const current = r[r.length - 1]; if (current > fib618) return { pred: "Xỉu", conf: 60 }; if (current < fib382) return { pred: "Tài", conf: 60 }; } return null; }
    ...
    tong_dao_dong() { if (this.tongSeq.length < 10) return null; const t = this.tongSeq.slice(-10); const m = avg(t); const last = t[t.length - 1]; if (Math.abs(last - m) < 1.5) return { pred: last < 10 ? "Tài" : "Xỉu", conf: 60 }; return null; }
    ...
    tong_support_resistance() { if (this.tongSeq.length < 30) return null; const t = this.tongSeq.slice(-30); const levels = []; for (let i = 2; i < t.length - 2; i++) { if (t[i] < t[i - 1] && t[i] < t[i + 1]) levels.push(t[i]); if (t[i] > t[i - 1] && t[i] > t[i + 1]) levels.push(t[i]); } if (levels.length > 0) { const last = t[t.length - 1]; const nearest = levels.reduce((a, b) => Math.abs(b - last) < Math.abs(a - last) ? b : a); if (Math.abs(last - nearest) <= 1) return { pred: last < nearest ? "Tài" : "Xỉu", conf: 65 }; } return null; }
    ...
    entropy_signal() { if (this.kqSeq.length < 20) return null; const e = entropy(this.kqSeq.slice(-20)); if (e < 0.5) return { pred: this.kqSeq[this.kqSeq.length - 1] === 1 ? "Tài" : "Xỉu", conf: 65 }; return null; }
    ...
    predict() {
        ...
        add(this.pattern_1_2_3(), 'pattern_1_2_3'); add(this.pattern_3_nhip(), 'pattern_3_nhip');
        ...
        add(this.has_1_and_6(), 'has_1_and_6'); add(this.has_1_and_2(), 'has_1_and_2');
        add(this.has_5_and_6(), 'has_5_and_6'); add(this.increasing_sequence(), 'increasing_sequence');
        ...
        add(this.markov_3(), 'markov_3'); add(this.markov_4(), 'markov_4');
        add(this.markov_5(), 'markov_5'); add(this.markov_weighted(), 'markov_weighted');
...
async function fetchData() {
    for (let attempt = 1; attempt <= 5; attempt++) {
        try {
            ...
            const raw = res.data;
            console.log(`📥 Response type: ${typeof raw}, isArray: ${Array.isArray(raw)}`);
            ...
            if (Array.isArray(raw)) {
                arr = raw;
                console.log(`✅ Format: array trực tiếp (${arr.length} items)`);
            }
            // Có thể bọc trong object
            else if (raw && typeof raw === 'object' && raw.data && Array.isArray(raw.data)) {
                arr = raw.data;
    ...
    try {
        ...
        if (currentPrediction && currentPrediction.Phien_hien_tai > 0 && latestPhien > oldPhien) {
            ...
            if (actual) {
                ...
                console.log(`📝 Phiên ${predictedPhien}: ${currentPrediction.Du_doan} vs ${actualStr} | ${addToHistory(predictedPhien, currentPrediction.Du_doan, actualStr, currentPrediction.Do_tin_cay) ? '✅' : '❌'}`);
            }
        }
...
(async () => {
    ...
    if (data && data.length >= 30) {
        gameHistory = data;
        console.log(`✅ Fetch OK: ${data.length} phiên`);
