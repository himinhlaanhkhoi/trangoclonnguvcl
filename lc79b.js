...
const normalize = item => {
    ...
    return { ket_qua: kq === 'tai' || kq === 'tài' ? 1 : 0, tong: item.point || 0, x1: (item.dices && item.dices[0]) || 0, x2: (item.dices && item.dices[1]) || 0, x3: (item.dices && item.dices[2]) || 0, phien: item.id || 0 };
};
...
// 30+ THUẬT TOÁN CHÍNH
// ============================================================
function algo_streak(h) { if (h.length < 3) return null; const tx = h.map(x => x.ket_qua === 1 ? 'T' : 'X'); const last = tx[tx.length - 1]; let s = 0; for (let i = tx.length - 1; i >= 0; i--) { if (tx[i] === last) s++; else break; } if (s >= 4) return last === 'T' ? 'T' : 'X'; if (s >= 2) return last; return null; }
function algo_alternating(h) { if (h.length < 6) return null; const tx = h.map(x => x.ket_qua === 1 ? 'T' : 'X'); for (let i = 1; i < 6; i++) if (tx[tx.length - i] === tx[tx.length - i - 1]) return null; return tx[tx.length - 1] === 'T' ? 'X' : 'T'; }
function algo_frequency(h) { if (h.length < 10) return null; const tx = h.map(x => x.ket_qua === 1 ? 'T' : 'X'); const tCount = tx.slice(-10).filter(t => t === 'T').length; if (tCount >= 7) return 'X'; if (tCount <= 3) return 'T'; return null; }
...
function algo_double_six(h) { if (h.length < 1) return null; const d = [h[h.length - 1].x1, h[h.length - 1].x2, h[h.length - 1].x3]; if (d.filter(x => x === 6).length >= 2) return 'T'; return null; }
function algo_double_one(h) { if (h.length < 1) return null; const d = [h[h.length - 1].x1, h[h.length - 1].x2, h[h.length - 1].x3]; if (d.filter(x => x === 1).length >= 2) return 'X'; return null; }
function algo_total_trend(h) { if (h.length < 5) return null; const t = h.map(x => x.tong).slice(-5); let up = true, down = true; for (let i = 1; i < 5; i++) { if (t[i] <= t[i - 1]) up = false; if (t[i] >= t[i - 1]) down = false; } if (up) return 'T'; if (down) return 'X'; return null; }
function algo_total_mean(h) { if (h.length < 20) return null; const t = h.map(x => x.tong); const m = avg(t.slice(-20)); const last = t[t.length - 1]; if (last > m + 2) return 'X'; if (last < m - 2) return 'T'; return null; }
function algo_total_touch(h) { if (h.length < 10) return null; const t = h.map(x => x.tong); const tx = h.map(x => x.ket_qua === 1 ? 'T' : 'X'); const last = t[t.length - 1]; if (last === 7 || last === 14) { const nexts = []; for (let i = 1; i < t.length; i++) if (t[i - 1] === last) nexts.push(tx[i] === 'T' ? 1 : 0); if (nexts.length >= 3) { const r = sum(nexts) / nexts.length; if (r > 0.65) return 'T'; if (r < 0.35) return 'X'; } } return null; }
function algo_bayes(h) { if (h.length < 20) return null; const b = h.map(x => x.ket_qua === 1 ? 1 : 0); const last = b[b.length - 1]; let same = 0, total = 0; for (let i = 1; i < b.length; i++) { if (b[i - 1] === last) { total++; if (b[i] === last) same++; } } if (total >= 5) { const p = same / total; if (p > 0.65) return last === 1 ? 'T' : 'X'; if (p < 0.35) return last === 1 ? 'X' : 'T'; } return null; }
function algo_pattern_match(h) { if (h.length < 25) return null; const b = h.map(x => x.ket_qua === 1 ? 1 : 0); const pat = b.slice(-8); const nexts = []; for (let i = 0; i < b.length - 9; i++) { if (JSON.stringify(b.slice(i, i + 8)) === JSON.stringify(pat)) nexts.push(b[i + 8]); } if (nexts.length >= 2) { const r = sum(nexts) / nexts.length; if (r >= 0.7) return 'T'; if (r <= 0.3) return 'X'; } return null; }
...
function algo_can_bang(h) { if (h.length < 20) return null; const b = h.map(x => x.ket_qua === 1 ? 1 : 0); const tai = sum(b.slice(-20)); if (tai >= 14) return 'X'; if (tai <= 6) return 'T'; return null; }
function algo_entropy_algo(h) { if (h.length < 20) return null; const b = h.map(x => x.ket_qua === 1 ? 1 : 0); const e = entropy(b.slice(-20)); if (e < 0.3) return b[b.length - 1] === 1 ? 'T' : 'X'; if (e > 0.9) return b[b.length - 1] === 1 ? 'X' : 'T'; return null; }
...
function algo_fourier(h) { if (h.length < 30) return null; const totals = h.map(x => x.tong); const N = Math.min(32, totals.length); const sig = totals.slice(-N); let maxM = 0, dF = 0; for (let k = 1; k <= N / 4; k++) { let real = 0, imag = 0; for (let n = 0; n < N; n++) { const angle = (2 * Math.PI * k * n) / N; real += sig[n] * Math.cos(angle); imag -= sig[n] * Math.sin(angle); } const mag = Math.sqrt(real * real + imag * imag); if (mag > maxM) { maxM = mag; dF = k; } } if (dF > 0) { const period = Math.round(N / dF); const phase = totals.length % period; return phase < period / 2 ? 'T' : 'X'; } return null; }
function algo_volume_profile(h) { if (h.length < 30) return null; const totals = h.map(x => x.tong); const profile = {}; totals.slice(-30).forEach(t => { const lvl = Math.round(t); profile[lvl] = (profile[lvl] || 0) + 1; }); let poc = null, mv = 0; for (const [l, v] of Object.entries(profile)) { if (v > mv) { mv = v; poc = parseFloat(l); } } const lt = totals[totals.length - 1]; if (poc && lt > poc + 1) return 'T'; if (poc && lt < poc - 1) return 'X'; return null; }
function algo_vwap(h) { if (h.length < 20) return null; const totals = h.map(x => x.tong); const vols = totals.map(t => Math.abs(t - 10.5) * 2); let num = 0, den = 0; for (let i = Math.max(0, totals.length - 20); i < totals.length; i++) { num += totals[i] * vols[i]; den += vols[i]; } const vwap = den > 0 ? num / den : 10.5; const lt = totals[totals.length - 1]; if (lt > vwap + 1) return 'T'; if (lt < vwap - 1) return 'X'; return null; }
function algo_genetic(h) { if (h.length < 30) return null; const tx = h.map(x => x.ket_qua === 1 ? 'T' : 'X'); const b = tx.map(t => t === 'T' ? 1 : 0); const feats = [avg(b.slice(-5)), avg(b.slice(-15)), entropy(b.slice(-20)), avg(b), Math.abs(avg(b) - 0.5)]; const weights = [0.4, 0.25, -0.2, 0.1, 0.05]; let sc = 0; for (let i = 0; i < feats.length; i++) sc += feats[i] * weights[i]; return sc >= 0.5 ? 'T' : 'X'; }

// ============ TẤT CẢ THUẬT TOÁN ============
const ALL_ALGORITHMS = [
    algo_streak, algo_alternating, algo_frequency, algo_markov,
    algo_triple, algo_double_six, algo_double_one,
    algo_total_trend, algo_total_mean, algo_total_touch,
    algo_bayes, algo_pattern_match, algo_cycle, algo_can_bang, algo_entropy_algo,
    algo_lstm, algo_attention, algo_gan, algo_dfa, algo_fourier,
    algo_volume_profile, algo_vwap, algo_genetic
];
...
console.log('   🧠 SIÊU THÔNG MINH ĐẲNG CẤP - 30+ THUẬT TOÁN 🧠');
