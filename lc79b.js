...
// ============================================================
// THE GOD PREDICTOR ULTIMATE V6.0 - 250+ THUẬT TOÁN
// ============================================================
...
async function fetchData() {
    for(let attempt=1;attempt<=5;attempt++){
        try{
            ...
            const res=await axios.get(API_URL,{timeout:20000,headers:{'User-Agent':'Mozilla/5.0','Accept':'application/json','Cache-Control':'no-cache'}});
            const raw=res.data;
            let arr=null;
            ...
            if(arr&&arr.length>=50){
                ...
                console.log(`✅ ${normalized.length} phiên, cuối: ${normalized[normalized.length-1].phien}`);
                return normalized;
            }
            console.log(`⚠️ Attempt ${attempt}: ${arr?arr.length:0} items (cần 50+)`);
...
async function updatePrediction(){
    ...
    try{
        ...
        // Kiểm tra kết quả dự đoán trước - QUAN TRỌNG: dùng đúng phiên
        if(currentPrediction&&currentPrediction.Phien_hien_tai>0){
            const predictedPhien=currentPrediction.Phien_hien_tai;
            // Tìm phiên có kết quả thực tế trong data mới fetch về
...
console.log('   👑 GOD PREDICTOR V6.0 - 250+ THUẬT TOÁN 👑');
console.log('   8 Nhóm | LSTM | GRU | Attention | CNN | Wavelet');
console.log('   API: wtxmd52.tele68.com/v1/txmd5/sessions');
console.log('   50 phiên gần nhất qua tất cả thuật toán');
console.log('='.repeat(70));

(async()=>{
    console.log('🔄 Fetch lần đầu (yêu cầu 50+ phiên)...');
    const data=await fetchData();
