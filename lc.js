...
function analyze(data, type) {
  if (!data || data.length < 18) {
    return {
      ...
      reasons: ['Cần ≥18 phiên đã hoàn thành'],
      experts: {},
      agree: '-'
    };
  }
  ...
  function expertPattern() {
    ...
    const details = ['Ptn' + bestLen + '→' + (next === 1 ? 'T' : 'X') + (memBoost > 0.3 ? '+Mem' : '')];
    return {
      pred: next,
  ...
  function expertDice() {
    ...
    if (recent.length < 12) {
      return { pred: 0, conf: 0.52, weight: dynamicWeight(1.7, 'dice'), details: ['Dice thiếu'], scoreT: 0.5, scoreX: 0.5 };
    }
    ...
    else if (avg12 <= 8.4) { scoreT += 2.25; details.push('Sum12 thấp'); }
    else if (avg12 >= 11.7) { scoreX += 1.1; details.push('Sum12 hơi cao'); }
    else if (avg12 <= 9.3) { scoreT += 1.1; details.push('Sum12 hơi thấp'); }
    if (avg8 >= 13.2) { scoreX += 1.4; details.push('Sum8 rất cao'); }
    else if (avg8 <= 7.8) { scoreT += 1.4; details.push('Sum8 rất thấp'); }
    if (highRatio >= 0.60) { scoreX += 1.6; details.push('Mặt cao'); }
    else if (lowRatio >= 0.60) { scoreT += 1.6; details.push('Mặt thấp'); }
    if (consecHigh >= 3) {
    ...
    if (lastSum >= 16) { scoreX += 1.2; details.push('Cực cao'); }
    else if (lastSum <= 5) { scoreT += 1.2; details.push('Cực thấp'); }
    const last16 = recent.slice(0, 16).map(d => d.result);
    const tai16 = last16.filter(x => x === 1).length;
    if (tai16 >= 12) { scoreX += 1.55; details.push('LệchT mạnh'); }
    else if (tai16 <= 4) { scoreT += 1.55; details.push('LệchX mạnh'); }
    const pred = scoreT >= scoreX ? 1 : 0;
    const total = scoreT + scoreX || 1;
    const dom = Math.abs(scoreT - scoreX) / total;
    return {
  ...
  if (wrong >= 3 && lastDir !== null) {
    ...
    factors.unshift('Đảo' + wrong + 'sai');
    reasons.push('Sai liên tiếp ' + wrong + ' lần → đảo chiều mạnh');
  ...
  if (agreeCount >= 4) reasons.push('Đồng thuận ' + agreeCount + '/5 chuyên gia');
...
<title>Phạm Khôi • VIP Adaptive</title>
...
<body class="px-3 py-5 max-w-md mx-auto">
  ...
  <div class="text-center text-[10px] text-white/12 pb-4 tracking-wide">Phạm Khôi • VIP Adaptive Memory Engine</div>
<script>
...
app.listen(PORT, '0.0.0.0', () => {
  console.log('');
  console.log('════════════════════════════════════════════');
  console.log('  PHẠM KHÔI • VIP Adaptive Memory Engine');
  console.log('  http://0.0.0.0:' + PORT);
  console.log('  Pattern Memory + Streak VIP + Dice Deep');
  console.log('  Càng treo lâu càng học càng khôn');
  console.log('════════════════════════════════════════════');
